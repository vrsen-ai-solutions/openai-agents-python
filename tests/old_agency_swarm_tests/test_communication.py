...
3. Only respond with 'error' if you detect multiple routing requests at once""",
    ...
    agency = Agency(
        ...
        # send_message_tool_class=SendMessageSwarm, # Removed - Internal handling
    )
...
@pytest.mark.asyncio
async def test_send_message_swarm(test_agents):
    ...
    while time.time() - start_time < timeout:
        try:
            response = await agency.get_completion("Hello, I need customer support please.")
            break
    ...
    response = await agency.get_completion("Who are you?")
...
@pytest.mark.asyncio
async def test_send_message_double_recipient_error(test_agents):
    ...
    agency = Agency([ceo, [ceo, customer_support]])
    response = await agency.get_completion(
