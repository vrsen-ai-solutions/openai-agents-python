...
def test_read_filesystem(agency):
    result = agency.get_completion(
        f"Use the list_directory tool to read the contents of {samples_dir} folder.",
        recipient_agent=agency.agents[0],
    )
    print(result)
...
def test_get_secret_word(agency):
    result = agency.get_completion(
        "Get secret word using get_secret_word tool.", recipient_agent=agency.agents[2]
    )
    print(result)
