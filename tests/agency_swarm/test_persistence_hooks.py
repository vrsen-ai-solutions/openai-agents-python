...
def mock_save_callback():
    return AsyncMock()  # save is awaited in on_run_end
...
def mock_thread_manager():
    ...
    manager._threads = {}
    # Removed get_thread mock as it might not be needed for these unit tests
    # add_items_and_save is part of ThreadManager but not directly used by PersistenceHooks itself
    # PersistenceHooks interacts with manager._threads directly (via context)
    return manager
...
class TestPersistenceHooksUnit:
    ...
    async def test_on_run_start_load_error(
        ...
        # Arrange
        load_error = IOError("Simulated load error")
    ...
    async def test_on_run_end_save_error(
        ...
        save_error = IOError("Simulated save error")
